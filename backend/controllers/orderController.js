...
exports.createOrder = async (req, res) => {
  try {
    ...
    const FREE_ABOVE = Number(process.env.FREE_SHIPPING_ABOVE || 499);
    const SHIP_CHARGE = Number(process.env.SHIPPING_CHARGE    || 49);
    const shippingTotal = itemsTotal >= FREE_ABOVE ? 0 : SHIP_CHARGE;
    const TAX_RATE  = Number(process.env.TAX_RATE_PERCENT || 18) / 100;
    const taxTotal      = Math.round(itemsTotal * TAX_RATE);
